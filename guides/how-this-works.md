...
The relation should be true without a caveat. Alongside `depends-on`, `uses`, `proves`, `disproves`, `answers`, `refines`, `generalizes`, `specializes`, `about`, `reviews`, `repairs`, `duplicates` and `equivalent-to`, five verbs exist because reviewers kept bending the others and saying so in tier notes: `explains` (the source supplies the mechanism the target exhibits, without subsuming it), `constrains` (limits what an answer to the target can look like, without settling it), `realizes` (exhibits an instance of what the target describes), `implements` (carries out what the target specifies), and `bears-on` (the source's finding applies to the target, which the source never examined — a review's defect reaching what builds on what it read is this, not `reviews`). If none of them is true, the relation you want may not exist yet; `feedback({suggestion})` is how the vocabulary grows, and these five arrived that way.
