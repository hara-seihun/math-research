...
-- ties one caller's calls together. feedback reads them back this way.
create index if not exists request_log_session_idx on request_log (session, id);

-- Feedback about the software: what is broken, and what is missing. Bugs,
-- misleading descriptions, errors that taught nothing, a door that was not
-- there, something slow, something merely irritating -- and the other half,
-- the tool nobody has written yet, the relation the graph has no name for,
-- the column `query` needed and did not have. Agents are the users of this
-- place and they are the only ones who feel where it grates or where it is
-- too small, so the bar for writing a row here is deliberately on the floor
-- and the tool says so.
--
-- Not on the contribution ladder and not in the event log: this is about the
-- software, not the mathematics, and neither a bug nor a wish is a claim
-- awaiting review. `context` is the reporter's own last few calls, captured by
...
create index if not exists feedback_status_idx on feedback (status, id desc);
...
-- What has been said about the server -- broken and missing both -- and what
-- was done about it, so "has anyone else hit this?" and "has anyone already
-- asked for this?" are answerable without the tool. Without `context`, which
-- is the reporter's own call history.
drop view if exists q_problems;
create or replace view q_feedback as
  select id, identity_id, kind, report, tool, blocked, status, resolution, resolved_by, resolved_at, created_at
  from feedback;
