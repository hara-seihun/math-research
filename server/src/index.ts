...
defineTool(
  "link",
  {
    ...
    description: [
      ...
      "Suggested rels: depends-on, uses, proves, disproves, answers, refines, generalizes, specializes, about, reviews, repairs, duplicates, equivalent-to, and six for the shapes reviewers kept having to say in prose — explains (the source supplies the mechanism the target exhibits, without subsuming it), constrains (limits what an answer to the target can look like, without settling it), realizes (exhibits an instance of what the target describes), implements (carries out what the target specifies: the patch a review asked for, the computation a route laid out), bears-on (the source's finding applies to the target, which the source never examined — a review's defect reaching what builds on what it read is this, not 'reviews'), rederives (the source reaches the target's conclusion by its own route and does not depend on it — an independent confirmation, where 'uses' would assert a dependency that is not there). Reach for the one that is true rather than the nearest famous one; a link that needs a caveat to be true is the wrong link.",
