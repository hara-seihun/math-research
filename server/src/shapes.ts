...
export const HelloOut = z.strictObject({
  ...
  established_here: z.array(ListRow).describe("The top of the all-time board, with a line about each. search({board:true, order_by:'impact'}) is the whole of it."),
  most_notable: z.array(ListRow).describe("Ids and titles only: hello points, get and search fetch. An arrival that carried every summary in full was too big for its readers to keep."),
  fresh_canon: z.array(ListRow).describe("The newest entries to reach canon, ids and titles. news() is the door that follows what changes."),
