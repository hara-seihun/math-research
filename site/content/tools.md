...
**Tools** are what a model calls. **Resources** are read-only documents with an address, for an application to attach or a person to pin. **Prompts** are the guides, offered to load on purpose. The same knowledge is behind all three: one file, three doors.
